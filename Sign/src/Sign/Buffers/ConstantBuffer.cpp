...
namespace Sign {
	ConstantBuffer::ConstantBuffer(uint32_t bufferSize, uint32_t heapIndex) : m_HeapIndex(heapIndex)
	{
		...
		m_ConstantBuffer = D3D12Utils::CreateBuffer(Renderer::GetContext()->GetDevice(), m_Size, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_GENERIC_READ, D3D12Utils::UploadHeapProps);

		m_ConstantBufferView.BufferLocation = m_ConstantBuffer->GetGPUVirtualAddress();
		m_ConstantBufferView.SizeInBytes = m_Size;

		Renderer::GetContext()->GetDevice()->CreateConstantBufferView(&m_ConstantBufferView, Renderer::GetContext()->GetCPUHandleAt(m_HeapIndex));
